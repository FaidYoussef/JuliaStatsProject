...
for i in 1:psARD1.k+1
    scatter!(times[i], observations[i], markersize = 4, label = false, color=:black)
end
# vline!(psARD1.maintenance_times, color=:red, label = "Maintenance")
# plot!(xlabel = "Temps", ylabel = "Degradation")
plot!()
# savefig("ARD1 IND_COL_1.png")
for i in 1:5000
...
histogram(estimations_rho_revised, bins=0:0.1:1, xlabel="Estimations", ylabel="Fréquence", title="Histogramme des estimations de rho", xlims=(0, 1),
          label="Moyenne = $(round(mean_estimation, digits=2)), Écart-type = $(round(std_estimation, digits=2))", legendfontsize=12, normalize=:pdf)
ylims!(0, 5)
xlims!(0, 1)
